...
vec2 getExtrusionOffset(vec2 line_clipspace,float offset_direction){
  ...
  vec2 offset_screenspace=dir_screenspace*offset_direction*strokeWidth/2.;
...
float getSegmentRatio(float index){
  return smoothstep(0.,1.,index/numSegments);
}

vec4 computeBezierCurve(vec4 source,vec4 target,vec4 controlPoint,float segmentRatio){
  float mt=1.-segmentRatio;
  float mt2=pow(mt,2.);
  float t2=pow(segmentRatio,2.);
  
  // quadratic curve
  float a=mt2;
  float b=mt*segmentRatio*2.;
  float c=t2;
...
void main(void){
  ...
  vec4 source=project_common_position_to_clipspace(vec4(sourcePos,1.));
  vec4 target=project_common_position_to_clipspace(vec4(targetPos,1.));
  vec4 controlPoint=project_common_position_to_clipspace(vec4(controlPointPos,1.));
